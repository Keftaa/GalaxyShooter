...
import systems.BulletsReloadSystem;
...
public class GameScreen implements Screen {
	...
	public void show() {
		...
		//main.engine.addSystem(new DamageSpriteSystem(renderer.batch));
		...
		main.engine.addSystem(new BulletsLauncherSystem());
		main.engine.getSystem(BulletsLauncherSystem.class).setProcessing(false);
		main.engine.addSystem(new BulletsReloadSystem(main.viewport, bodyGenerator));
		...
		main.engine.addSystem(new SpriteRenderingAfterLightSystem(
				main.batch));
	...
	public void render(float delta) {
		...
		for (EntitySystem system : main.engine.getSystems())
			if(system.checkProcessing())
				if (!(system instanceof SpriteRenderingAfterLightSystem) && !(system instanceof ParticleRenderingSystem))
					system.update(delta);

		main.batch.end();

		lightsManager.handler.setCombinedMatrix( (OrthographicCamera) main.viewport.getCamera());
		...
		main.batch.begin();
		main.engine.getSystem(ParticleRenderingSystem.class).update(
				delta);
